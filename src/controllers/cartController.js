  ...
  try {
    ...
    if (!productByProductId) {
      return res
        ...
        .send({
          status: false,
          message: `No product found by ${productId}`
        });
    }
    ...
    if (requestBody.hasOwnProperty("cartId")) {
      ...
      if (!validator.isValid(cartId)) {
        return res
          ...
          .send({
            status: false,
            message: "cartId could not be blank"
          });
      }
      ...
      if (!cartByCartId) {
        return res
          ...
          .send({
            status: false,
            message: `No cart found by ${cartId}`
          });
      }

      // checking whether user has any cart
      const cartByUserId = await CartModel.findOne({
        userId: userId
      });
...
const updateCart = async function (req, res) {
  try {
    ...
    if (![0, 1].includes(removeProduct)) {
      return res.status(400).send({
        status: false,
        message: "RemoveProduct is required and its value must be either 0 or 1",
      });
    }
